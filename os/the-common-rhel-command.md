...
- 网络状态（netstat）
  ...
  ```
  netstat -napo | grep 1080 | wc -l
  ```

  查看连接某服务端口最多的的IP地址：

  ```
  netstat -nat | grep "192.168.1.15:22" |awk '{print $5}'|awk -F: '{print $1}'|sort -nr|uniq -c|head -20
  ```
...
- 文件定位（whereis/find）
  ...
  find . -type f -exec grep -l "10.10.20.90" {} \	# 查找当前目录下包含10.10.20.90的所有文find ./ -type f -name "*.*" | xargs grep -rnl  "Hello" # 查找当前目录含有Hello字符串的所有文件，r递归，n显示行号，l只显示文件名
